...
> **source_blob_sha:** `9027c38069811909eb1a16df3fc3362714506628`
>
...
## 24.3. Межрепозиторная доставка контента начинается со сравнения

При переносе или построении контента между:
- каноническим источником и интерфейсными проекциями;
...
# 25. Производные состояния и проекции нельзя дублировать вручную

Самая сложная часть проекта — состояния во времени. Поэтому они авторски описываются только один раз.

Не поддерживать вручную независимые копии для Главы 1 / Главы 2:
- состояния персонажа;
- состояния мира;
- знания игрока;
...
если их можно вывести из событий, временных состояний, изменений знания, правил доступности и выбранной точки среза.
...
Срез — координата проекции, а не ручная копия всей базы.

Временную оболочку использовать только для свойств, которые действительно меняются.

Если событие может быть первичным владельцем изменения состояния, предпочитать событие и его effect, а не несколько ручных копий одного изменения.

Интерфейсный репозиторий получает производные проекции и не создаёт параллельную авторскую модель состояний.

---

# 26. Архитектура парных репозиториев

У проекта два репозитория с разными обязанностями, но только одна авторская база.

## 26.1. Канонический репозиторий контента

[bestkvestnn-pixel/dimnadvod](https://github.com/bestkvestnn-pixel/dimnadvod) — AUTHOR / CANON / KNOWLEDGE SOURCE.

Он владеет:
- фактами мира;
- сущностями и стабильными ID;
- хронологией и временными состояниями;
- событиями, сценами, процессами и историями;
- знанием и интерпретациями;
- главами, переходами, срезами и доступностью;
- исходными игровыми материалами и assets;
- схемами экспорта/проекций.

## 26.2. Интерфейсный репозиторий

[bestkvestnn-pixel/test-app](https://github.com/bestkvestnn-pixel/test-app) — INTERFACE REPOSITORY.

Его корневая инструкция:
...
Он не ведёт независимую авторскую копию сюжетной базы.

## 26.3. Направление контента

```text
dimnadvod
  SOURCE / CANON
      ↓
версионированная производная проекция
      ↓
test-app
  INTERFACES + RUNTIME
```

Это не модель `database A ⇄ database B`.

## 26.4. Записи через Author Studio

Авторский интерфейс может инициировать изменение, но запись должна попасть непосредственно в `dimnadvod` по правилам корневого и локального `AGENTS.md`.
...
Это не обратная синхронизация.

## 26.5. Канонический контракт доставки контента

Нормативный межрепозиторный контракт:

[contracts/content-delivery/AGENTS.md](../contracts/content-delivery/AGENTS.md)

Старый [sync/AGENTS.md](../sync/AGENTS.md) оставлен только как совместимый указатель.

Перед межрепозиторным обновлением читать:
1. корневой `/AGENTS.md`;
...
Если меняется общая граница — роли репозиториев, состояния, ID, Author Studio, доставка контента, content lock, generated/runtime — в той же операции проверить все три файла.

Не делать корневые инструкции одинаковыми: общие правила живут в контракте, локальные обязанности — в корнях.

При противоречии межрепозиторную доставку остановить до согласования.

---

# 27. После любой операции
...
# 28. Краткая формула архитектуры
...
> **dimnadvod — единственная авторская база; test-app — интерфейсы и runtime.**
